...
if (!TOKEN || !CLIENT_ID || !GUILD_ID || !STAFF_ROLE_ID) {
  console.error("Faltam variáveis: BOT_TOKEN, CLIENT_ID, GUILD_ID, STAFF_ROLE_ID");
  process.exit(1);
}
...
const hasStaff = (member) => member?.roles?.cache?.has(STAFF_ROLE_ID);
const ownerFromTopic = (topic="") => (topic.match(/ticketOwner:(\d+)/)?.[1]) || null;

// ⚠️ IMPORTANTE: MessageContent para ler links enviados pelos usuários
...
async function registerCommands() {
  const cmds = [
    new SlashCommandBuilder()
      .setName("gmp")
      .setDescription("Lê a última Gamepass enviada no ticket, calcula e registra nos logs"),
  ].map(c => c.toJSON());

  const rest = new REST({ version: "10" }).setToken(TOKEN);
  await rest.put(Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID), { body: cmds });
  console.log("✅ /gmp registrado");
}

// pega o último link de gamepass do ticket
async function findLastGamepassLink(channel) {
  const msgs = await channel.messages.fetch({ limit: 50 });
  for (const [, m] of msgs) {
    const content = m.content || "";
    const match = content.match(/https?:\/\/www\.roblox\.com\/pt\/game-pass\/\d+\/[^\s]+/i);
    if (match) return match[0];
  }
  return null;
}

// pega nome + robux (somente o número visível que aparece na página)
async function getGamepassNameAndRobux(url) {
  const res = await fetch(url, { headers: { "user-agent": "Mozilla/5.0" } });
  if (!res.ok) throw new Error("Falha ao abrir a página");
  ...
  // Procura algo como "144 Robux" no texto da página
  const text = $.text();
...
client.on("interactionCreate", async (i) => {
  if (!i.isChatInputCommand() || i.commandName !== "gmp") return;

  try {
    if (!hasStaff(i.member)) return i.reply({ content: "❌ Sem permissão.", ephemeral: true });

    const ownerId = ownerFromTopic(i.channel?.topic || "");
    if (!ownerId) return i.reply({ content: "❌ Use /gmp dentro de um ticket criado pelo bot.", ephemeral: true });

    await i.deferReply({ ephemeral: true });

    const link = await findLastGamepassLink(i.channel);
    if (!link) return i.editReply("❌ Não achei link de Gamepass no ticket (cliente precisa mandar o link).");
    ...
    try {
      ...
      console.error("Falha ao dar cargo comprador:", e?.message || e);
    }
