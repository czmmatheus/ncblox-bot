...
const CFG = {
  ...
  RATE_PER_1000: 28,
  PRICE_MULT: 1.3, // com taxa no robux (não usar no /gmp)
  PURPLE: 0x7c3aed,
...
/* =======================
   CLIENT + INTENTS
   (PRECISA MessageContent pro /gmp achar link)
======================= */
...
const ownerFromTopic = (topic = "") =>
  topic.match(/ticketOwner:(\d+)/)?.[1] || null;

const priceRobuxOrderBRL = (robux, withTax) => {
...
async function scheduleAutoClose(channel, openedAt) {
  ...
  const t = setTimeout(async () => {
    try {
      await channel.send(
        "⏳ Ticket encerrado automaticamente após **24 horas**."
      );
    } catch {}
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        `• Com taxa (+30%) → **1000 = ${brl(
          CFG.RATE_PER_1000 * CFG.PRICE_MULT
        )}**`,
        "",
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "",
        "Calcule o valor exato antes de comprar.",
...
/* =======================
   /logs -> extrair pedido de ROBUX do embed
======================= */
async function extractRobuxOrder(channel) {
  ...
  for (const [, msg] of msgs) {
    ...
    const modo = (desc.match(/\*\*Modo:\*\*\s*(.+)/i)?.[1] || "—")
      .split("\n")[0]
      .trim();
...
/* =======================
   /gmp -> achar ultimo link e ler robux da pagina
   REGRA: SEM TAXA, só Robux/1000*28
======================= */
async function findLastGamepassLink(channel) {
  ...
  for (const [, m] of msgs) {
    ...
    const match = content.match(
      /https?:\/\/www\.roblox\.com\/pt\/game-pass\/\d+\/[^\s]+/i
    );
...
async function getGamepassNameAndRobux(url) {
  ...
  // pega algum trecho "### Robux"
  const text = $.text();
  const m = text.match(/(\d{1,6})\s*Robux/i);
  if (!m) throw new Error("Não achei o número de Robux na página");

  const robux = Number(m[1]);
  if (!Number.isFinite(robux) || robux <= 0)
    throw new Error("Robux inválido");
...
async function registerCommands() {
  const commands = [
    ...
    new SlashCommandBuilder().setName("logs").setDescription("Finaliza ticket de Robux: registra, dá cargo e fecha (staff)"),
    new SlashCommandBuilder().setName("gmp").setDescription("Finaliza ticket de Gamepass: lê link, calcula, registra e fecha (staff)"),
  ].map((c) => c.toJSON());
  ...
  try {
    ...
    if (i.isChatInputCommand()) {
      ...
      if (i.commandName === "logs") {
        ...
        if (!order) {
          return i.editReply("❌ Não encontrei o pedido de Robux nesse ticket.");
        }
        ...
        try {
          ...
          if (member && !member.roles.cache.has(CFG.BUYER_ROLE_ID)) {
            await member.roles.add(CFG.BUYER_ROLE_ID, "Compra registrada via /logs");
          }
          buyerOk = true;
        } catch {}
        ...
        await i.channel.send(
          `✅ Venda registrada por <@${i.user.id}>.\n` +
            `🏷️ Cargo **Comprador** ${buyerOk ? "aplicado" : "NÃO aplicado (permissão/hierarquia)"} para <@${ownerId}>.\n` +
            `🔒 Ticket será fechado em 5 segundos...`
        );

        await i.editReply("✅ Registrado. Fechando ticket...");
        return deleteTicket(i.channel, "Finalizado via /logs");
      }

      // /gmp (Gamepass ticket - lê link)
      if (i.commandName === "gmp") {
        ...
        // SEM TAXA no gmp
        const total = round2((robux / 1000) * CFG.RATE_PER_1000);
        ...
        try {
          const member = await i.guild.members.fetch(ownerId);
          if (member && !member.roles.cache.has(CFG.BUYER_ROLE_ID)) {
            await member.roles.add(CFG.BUYER_ROLE_ID, "Compra Gamepass registrada via /gmp");
          }
          buyerOk = true;
        } catch {}

        await i.channel.send(
          `✅ Gamepass registrada por <@${i.user.id}>.\n` +
            `🏷️ Cargo **Comprador** ${buyerOk ? "aplicado" : "NÃO aplicado (permissão/hierarquia)"} para <@${ownerId}>.\n` +
            `🔒 Ticket será fechado em 5 segundos...`
        );
    ...
    if (i.isButton()) {
      ...
      if (i.customId === "close_ticket") {
        const ownerId = ownerFromTopic(i.channel?.topic || "");
        const can = hasStaff(i.member) || (ownerId && i.user.id === ownerId);
    ...
    if (i.isModalSubmit()) {
      ...
      if (i.customId.startsWith("calc_modal:")) {
        ...
        if (!Number.isFinite(robux) || robux <= 0) {
          return i.reply({ content: "❌ Robux inválido.", ephemeral: true });
        }

        const total = round2(priceRobuxOrderBRL(robux, withTax));

        const embed = new EmbedBuilder()
          .setColor(CFG.PURPLE)
          .setTitle("🧮 Resultado da calculadora — " + BRAND)
          .setDescription(
            [
              `**Robux:** ${robux}`,
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            [
              ...
              "⏳ **Aguarde até 1 dia (24h)**. Após esse tempo o ticket fecha automaticamente.",
              "",
              "✅ Ao finalizar: staff pode usar **/gmp** se o cliente mandar o link da Gamepass do Roblox no ticket.",
            ].join("\n")
          );
